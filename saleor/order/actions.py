...
def order_created(
    ...
    payload = {
        ...
        "delivery_fee": "{symbol} {price}".format(symbol=channel_symbol, price=Decimal(order.delivery_fee).quantize(TWOPLACES)) if order.delivery_fee > 0 else 0,
        "transaction_cost": "{symbol} {price}".format(symbol=channel_symbol, price=Decimal(order.transaction_cost).quantize(TWOPLACES)) if order.transaction_cost > 0 else 0,
